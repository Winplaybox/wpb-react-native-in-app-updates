...
const { WpbInAppUpdates } = NativeModules;
const WpbInAppUpdatesOrEmpty: {
...
export default class InAppUpdates extends InAppUpdatesBase {
  constructor() {
    super();
    this.eventEmitter = new NativeEventEmitter(WpbInAppUpdates);
    this.eventEmitter.addListener(
      WpbInAppUpdatesOrEmpty?.IN_APP_UPDATE_STATUS_KEY,
      this.onIncomingNativeStatusUpdate
    );
    this.eventEmitter.addListener(
      WpbInAppUpdatesOrEmpty?.IN_APP_UPDATE_RESULT_KEY,
  ...
  public addStatusUpdateListener = (callback: AndroidStatusEventListener) => {
    ...
    if (this.statusUpdateListeners.hasListeners()) {
      WpbInAppUpdates.setStatusUpdateSubscription(true);
    }
  };
  ...
  public removeStatusUpdateListener = (
    ...
    if (!this.statusUpdateListeners.hasListeners()) {
      WpbInAppUpdates.setStatusUpdateSubscription(false);
    }
  };
  ...
  public checkNeedsUpdate = (
    ...
    return WpbInAppUpdates.checkNeedsUpdate()
  ...
  public startUpdate = (
    ...
    return WpbInAppUpdates.startUpdate(updateType).catch((err: any) => {
      this.throwError(err, 'startUpdate');
    });
  };

  public installUpdate = (): void => {
    WpbInAppUpdates.installUpdate();
  };
}
